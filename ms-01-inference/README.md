...
| `inf-04-quantifier` | added | Measure a model's quality by its perplexity on a fixed corpus, measure what reduced precision gains in decoding while gaining nothing in prefill, and settle between the two against a budget set in advance | not started · 4 exercises |
